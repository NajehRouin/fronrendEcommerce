...
const Cart = () => {
  ...
  const PlaceOrder = async (e) => {
    e.preventDefault();
    try {
      const placeOrder = await fetch(Api.createOrder.url, {
        method: Api.createOrder.method,
        credentials: "include",
      });

      const dataResponse = await placeOrder.json();
    ...
    <div className="container mx-auto">
      <div className="text-center text-lg my-3">
        {data.length === 0 && !loading && (
          <p className="bg-white py-5">𝕐𝕠𝕦𝕣 𝕤𝕙𝕠𝕡𝕡𝕚𝕟𝕘 𝕔𝕒𝕣𝕥 𝕚𝕤 𝕖𝕞𝕡𝕥𝕪</p>
        )}
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-10 lg:justify-between p-4">
        ...
        <div className="mt-5 lg:mt-0 w-full max-w-sm rounded-md">
          {data.length === 0 && !loading ? (
            ...
            <div className="h-38 bg-white rounded-md  shadow-lg p-4">
            <h2 className="text-white text-center bg-red-600 px-2 py-1 rounded-md">Summary</h2>
            ...
            <button
              onClick={PlaceOrder}
              className="bg-blue-600 p-2 text-white w-full mt-2 rounded-md"
            >
              Place Order
            </button>
          </div>
